...
def instance_segmentation(points, chunk_id=0):
    ...
    sp_indices_generator = build_superpoints(points)

    superpoints = []
    for i, idx in enumerate(sp_indices_generator):
        print("sp_indices", i)
    ...
    for sp_id, idx in enumerate(sp_indices_generator):
        print("sp_id", sp_id, "idx", idx)
